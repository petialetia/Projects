...
DEF_CMD (ZeroNodeCorrupted, /*(list->data[0].val != POISON) ||*/ (list->data[0].next != 0) || (list->data[0].prev != 0), "Zero node is corrupted")
...
DEF_CMD (IncorrectFreeNode, (list->size < list->capacity) && ((list->data[list->free].prev != -1)/* || (list->data[list->free].val != POISON)*/), "Free node is incorrect")
