...
Программа анализирует текс из Text.txt и создаёт файл Translated.html, который можно открыть в браузере. При наведении на слово, если оно было переведено, будет высвечена вырезка из переводчика, иначе будет выведено сообщение "ERROR". 
...
Здесь функция 0x0000000000002de0 - это FindBucket
...
6) Переписывание FindHashTable на ассемблере в отдельном файле
